...
echo pupaos-live > "${NEWROOT}/etc/hostname"
...
chroot "${NEWROOT}" sh -c 'echo "root:pupaos" | chpasswd -c SHA512'
chroot "${NEWROOT}" sh -c "echo '${USERNAME}:pupaos' | chpasswd -c SHA512"

# Enable sudo permission by default.
if [ -f "${NEWROOT}/etc/sudoers" ]; then
    echo "${USERNAME} ALL=(ALL:ALL) NOPASSWD: ALL" > "${NEWROOT}/etc/sudoers.d/99-pupaos-live"
fi

if [ -d "${NEWROOT}/etc/polkit-1" ]; then
    # If polkit is installed allow users in the wheel group to run anything.
    cat > ${NEWROOT}/etc/polkit-1/rules.d/pupaos-live.rules <<_EOF
...
_EOF
    chroot "${NEWROOT}" chown polkitd:polkitd /etc/polkit-1/rules.d/pupaos-live.rules
fi
